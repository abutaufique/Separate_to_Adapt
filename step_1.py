...
while k <500:
    for (i, ((im_source, label_source), (im_target, label_target))) in enumerate(
        ...
        im_source = torch.from_numpy(im_source).to(device)
        label_source = torch.from_numpy(label_source).to(device)
        im_target = torch.from_numpy(im_target).to(device)
        ...
        if log.step % 10 == 1:
            ...
            acc_train = torch.from_numpy(np.asarray([counter.reportAccuracy()], dtype=np.float32)).to(device)
...
while k <400:
    for (i, ((im_source, label_source), (im_target, label_target))) in enumerate(
        ...
        d2 += CrossEntropyLoss(torch.from_numpy(np.concatenate((np.zeros((2,1)), np.ones((2,1))), axis =
            -1).astype('float32')).to(device),pred01)
